...
\def\kDObject#1\withNode#2\GO%
  {\node[kD/.cd, every object, self naming node, node contents={$#1$}, #2];}

\def\kDMorphism#1\withNode#2\withEdge#3\from#4\to#5\GO%
  {\path (#4) edge [kD/.cd, every arrow, #3,
              edge node={node [kD/.cd, every label, self naming node, node contents={$#1$}, #2]}]
         (#5);}
...
\def\DoMorphism #1 #2 #3\GO%
  {\kDTrisect#2\at:\into EdgeSpec\and MathSpec\and NodeSpec\GO%
   ...
   \expandafter\kDMorphism\expandafter\expandafter\expandafter\MathSpec%
   \expandafter\expandafter\expandafter\withNode\expandafter\NodeSpec%
